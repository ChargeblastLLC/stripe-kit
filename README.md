...
| `@LossyList` on the `data` array of every list and search response | One undecodable record can no longer abort the rest of a page. Lossy only where the list IS the response; nested in another object (`Customer.subscriptions`, `Subscription.items`) it stays strict, because shortening a relation silently changes what the caller decides. A page that loses every record still throws |
...
| `StripeDecodingReport` carries `failureKind` / `failureCodingPath` alongside `failureDescription` | The description is `String(describing:)` of whichever error arrived, whose text differs between Darwin Foundation and swift-corelibs-foundation. A consumer that parsed it to group failures worked on macOS and produced one group per record on Linux. A record that fails on something that never became a `DecodingError` (an integer overflowing `Int`, for instance) reports `nonDecodingError` and falls back to the record path, so the grouping key is never empty |
